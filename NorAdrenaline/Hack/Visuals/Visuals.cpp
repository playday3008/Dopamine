...
void CVisuals::DrawHistory(int i)
{
	if (cvar.esp_drawhistory)
	{
		...
		if (m_bScreenTop && m_bScreenBot)
		{
			...
			float PlayerBoxHeight = g_Player[i].bDucked ? Height : Height * 0.9f;

			if (cvar.esp_box == 1)
			{
				g_Drawing.Box(int(ScreenTop[0] - (PlayerBoxHeight * 0.25f)), int(ScreenTop[1]), int(PlayerBoxHeight / 2),
					int(PlayerBoxHeight), 1, r, g, b, 255);
				if (cvar.esp_box_outline)
				{
					g_Drawing.BoxOutline(int(ScreenTop[0] - (PlayerBoxHeight * 0.25f)), int(ScreenTop[1]), int(PlayerBoxHeight / 2),
						int(PlayerBoxHeight), 1, r, g, b, 255);
				}
			}
			else if (cvar.esp_box == 2)
			{
				g_Drawing.DrawCoalBox(int(ScreenTop[0] - (PlayerBoxHeight* 0.25f)), int(ScreenTop[1]), int(PlayerBoxHeight / 2),
					int(PlayerBoxHeight), 1, r, g, b, 255);
				if (cvar.esp_box_outline)
				{
					g_Drawing.DrawOutlineCoalBox(int(ScreenTop[0] - (PlayerBoxHeight* 0.25f)), int(ScreenTop[1]), int(PlayerBoxHeight / 2),
						int(PlayerBoxHeight), 1, r, g, b, 255);
				}
			}
			else if (cvar.esp_box == 3)
			{
				g_Drawing.BoxCorner(int(ScreenTop[0] - (PlayerBoxHeight* 0.25f)), int(ScreenTop[1]), int(PlayerBoxHeight / 2),
					int(PlayerBoxHeight), 1, r, g, b, 255);
				if (cvar.esp_box_outline)
				{
					g_Drawing.BoxCornerOutline(int(ScreenTop[0] - (PlayerBoxHeight* 0.25f)), int(ScreenTop[1]), int(PlayerBoxHeight / 2),
						int(PlayerBoxHeight), 1, r, g, b, 255);
				}
			}
...
void TriggerStatus()
{
	...
	static bool oldAimbot = g_AimBot.AimbotKeyStatus;
...
void CVisuals::CustomFOV()
{
		...
		else if (!g_Local.bScoped && !g_pGlobals.bSnapshot && !g_pGlobals.bScreenshot && !cvar.hide_from_obs)
		{
			fov = cvar.fov_zoom;
		}
...
void CVisuals::PlayerESP(unsigned int i)
{
	...
	if (m_bScreenTop && m_bScreenBot)
	{
		...
		g_Drawing.FillArea(int(ScreenTop[0] - (PlayerBoxHeight * 0.25f)), int(ScreenTop[1]), int(PlayerBoxHeight / 2),
			int(PlayerBoxHeight), r, g, b, cvar.esp_boxfill);
		char *szWeapon = g_PlayerExtraInfoList[i].szWeaponName;
